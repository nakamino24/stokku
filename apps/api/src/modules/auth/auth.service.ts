...
export const AuthService = {
  async register(data: { email: string; password: string; name: string; organizationName: string }) {
    ...
    return (prisma.$transaction as any)(async (tx: any) => {
      ...
      return { user: payload, accessToken: generateAccessToken(payload), refreshToken: session.refreshToken };
    });
  },
  ...
  async refresh(refreshToken: string) {
    ...
    if (session.revokedAt) {
      ...
      const isRecentRotation = session.revokedReason === 'ROTATED' && now.getTime() - session.revokedAt.getTime() <= graceMs;
      if (isRecentRotation) throw new AppError(409, 'Refresh credential was already rotated', 'REFRESH_ALREADY_ROTATED');
    ...
    await (prisma.$transaction as any)(async (tx: any) => {
      ...
      if (consumed.count !== 1) throw new AppError(409, 'Refresh credential was already rotated', 'REFRESH_ALREADY_ROTATED');
