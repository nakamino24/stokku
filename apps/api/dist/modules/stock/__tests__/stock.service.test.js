"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const stock_service_1 = require("../stock.service");
jest.mock('@stokku/database', () => ({
    prisma: {
        stockLevel: {
            findMany: jest.fn(),
            findFirst: jest.fn(),
            count: jest.fn(),
            upsert: jest.fn(),
            update: jest.fn(),
        },
        stockMovement: {
            findMany: jest.fn(),
            count: jest.fn(),
            create: jest.fn(),
        },
        product: { findFirst: jest.fn() },
        warehouse: { findFirst: jest.fn() },
        $transaction: jest.fn(),
        $queryRaw: jest.fn(),
    },
}));
jest.mock('../../../config', () => ({
    config: {},
}));
const { prisma } = jest.requireMock('@stokku/database');
function mockTx(fn) {
    const tx = {
        stockLevel: { findFirst: prisma.stockLevel.findFirst, upsert: prisma.stockLevel.upsert, update: prisma.stockLevel.update },
        product: { findFirst: prisma.product.findFirst },
        warehouse: { findFirst: prisma.warehouse.findFirst },
        stockMovement: { create: prisma.stockMovement.create },
    };
    return Promise.resolve(fn(tx));
}
describe('StockService', () => {
    beforeEach(() => jest.clearAllMocks());
    describe('adjust', () => {
        it('should upsert stock level and create movement', async () => {
            prisma.product.findFirst.mockResolvedValue({ id: 'prod-1' });
            prisma.warehouse.findFirst.mockResolvedValue({ id: 'wh-1' });
            prisma.stockLevel.upsert.mockResolvedValue({ id: 'sl-1', quantity: 20, available: 20 });
            prisma.$transaction.mockImplementation(mockTx);
            const result = await stock_service_1.StockService.adjust('org-1', 'user-1', {
                productId: 'prod-1', warehouseId: 'wh-1', quantity: 20,
            });
            expect(result).toBeDefined();
            expect(prisma.stockLevel.upsert).toHaveBeenCalled();
        });
        it('should throw for zero quantity', async () => {
            await expect(stock_service_1.StockService.adjust('org-1', 'user-1', {
                productId: 'prod-1', warehouseId: 'wh-1', quantity: 0,
            })).rejects.toThrow('Quantity must be non-zero');
        });
        it('should throw for invalid product', async () => {
            prisma.product.findFirst.mockResolvedValue(null);
            await expect(stock_service_1.StockService.adjust('org-1', 'user-1', {
                productId: 'invalid', warehouseId: 'wh-1', quantity: 10,
            })).rejects.toThrow('Product not found');
        });
    });
    describe('transfer', () => {
        const transferData = {
            productId: 'prod-1', fromWarehouseId: 'wh-1', toWarehouseId: 'wh-2', quantity: 10,
        };
        it('should transfer stock between warehouses', async () => {
            prisma.stockLevel.findFirst.mockResolvedValueOnce({ id: 'sl-1', quantity: 50, available: 50 });
            prisma.stockLevel.upsert.mockResolvedValue({ id: 'sl-2', quantity: 10, available: 10 });
            prisma.stockLevel.update.mockResolvedValue({});
            prisma.$transaction.mockImplementation(mockTx);
            const result = await stock_service_1.StockService.transfer('org-1', 'user-1', transferData);
            expect(result).toBeDefined();
            expect(prisma.stockLevel.update).toHaveBeenCalled();
            expect(prisma.stockLevel.upsert).toHaveBeenCalled();
        });
        it('should throw for same warehouse transfer', async () => {
            await expect(stock_service_1.StockService.transfer('org-1', 'user-1', { ...transferData, toWarehouseId: 'wh-1' }))
                .rejects.toThrow('Source and destination');
        });
        it('should throw for insufficient stock', async () => {
            prisma.stockLevel.findFirst.mockResolvedValueOnce({ id: 'sl-1', quantity: 5, available: 5 });
            prisma.$transaction.mockImplementation(mockTx);
            await expect(stock_service_1.StockService.transfer('org-1', 'user-1', transferData))
                .rejects.toThrow('Insufficient stock');
        });
    });
});
//# sourceMappingURL=stock.service.test.js.map