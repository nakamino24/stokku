...
import { FiBox, FiCheckCircle, FiAlertTriangle, FiX, FiMapPin } from 'react-icons/fi';
...
interface PickTask {
  id: string;
  status: 'READY' | 'CLAIMED' | 'PICKED' | 'SHORTED';
  warehouseId: string;
  productId: string;
...
export default function PickingPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<Paginated<PickTask>>(`/picking?${params}`, fetcher);
  ...
  const claim = async (id: string) => {
    setActionError(null);
    try {
      await api.post(`/picking/${id}/claim`);
      await mutate();
      setFeedback('Pick task claimed.');
      setSelectedId(null);
    } catch (err) {
  ...
  const confirm = async (id: string) => {
    ...
    try {
      ...
      setActionError(err instanceof Error ? err.message : 'Unable to confirm pick.');
    }
  };
  ...
  const shortPick = async (id: string) => {
    ...
    try {
      ...
      setActionError(err instanceof Error ? err.message : 'Unable to record short pick.');
    }
  };
    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Fulfill allocated sales orders from the pick face</p>
        </div>
      </div>

      <div className="mb-4 flex gap-2 flex-wrap">
        {['', 'READY', 'CLAIMED', 'PICKED', 'SHORTED'].map((status) => (
          <button
            key={status}
      ...
      {error && <div className="p-4 bg-red-50 text-red-700 rounded-lg mb-4">Failed to load picking tasks</div>}
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="text-lg">No picking tasks</p>
          <p className="text-sm mt-2">Allocated sales orders will appear here for pick execution.</p>
        </div>
      )}

      {data?.pagination && <Pagination page={page} totalPages={data.pagination.totalPages} onPageChange={setPage} label="Picking pages" />}
