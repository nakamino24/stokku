...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (router.query.registered === '1') setVerificationNotice(true)
    ...
    <AuthLayout>
      <div className="space-y-8">
        ...
        {verificationNotice && (
          <div
            role="status"
            className="rounded-lg border border-indigo-100 bg-indigo-50 p-3 text-sm text-indigo-700"
          >
            Account created. Check your email to verify it before signing in.
            {' '}
            <Link className="font-semibold underline" href="/auth/verify-email">Resend verification email</Link>
          </div>
        )}
