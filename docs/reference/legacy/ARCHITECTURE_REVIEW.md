# Historical: Superseded Architecture Review

> This is a dated review of an earlier project-management implementation. It is
> retained for evidence only. The current baseline is `docs/architecture.md` and
> the release status is tracked in `docs/ProductionReadiness.md`.
...
- **Details**:
  ...
  - New and changed passwords use Argon2id; legacy bcrypt hashes are opportunistically
    rehashed after successful login during the migration window.
