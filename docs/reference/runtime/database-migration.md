# Reference Runtime: Prisma PostgreSQL Migration Runbook

> This runbook applies only to the active Prisma reference implementation. The
> target database principles are defined in `docs/Database.md`; any Drizzle/schema
> transition requires an approved ADR and tested vertical slice.
...
Do not run the seed script against production. Use it only against an explicitly
